...
classifier_name=st.sidebar.selectbox('select classifier',('','Randomforest','naive bayes','logistic regresssion','DecisionTreeClassifier'))
...
if classifier_name=='Randomforest':
	...
	FP = cm[0][1]
	st.write(cm)
	st.write('Accuracy is of random forest = "{}!"'.format(  (TP + TN) / (TP + TN + FN + FP)))
...
if classifier_name=='logistic regresssion':
	...
	fit = rfe.fit(X_train_nf, y_train_nf)
	model.fit(X_train_nf, y_train_nf)
	y_pred =model.predict(X_test_nf)


from sklearn import metrics
confusion=metrics.confusion_matrix(y_test_nf,y_pred)
st.write(confusion)
from sklearn.metrics import classification_report
st.write(classification_report(y_test_nf, y_pred))
